                                ...
                                <div class="options">
                                    <div class="btn-group">
                                        <form method="POST" action="{{ route('promoteUser', ['id' => $user->id]) }}">
                                            {{ method_field('PATCH') }}
                                            {{ csrf_field() }}
                                            <button type="submit" class="btn btn-outline-primary" {{ $user->role->id === 1 ? 'disabled' : '' }}><i class="fad fa-fw fa-arrow-up"></i> Promote</button>
                                        </form>
                                        <form method="POST" action="{{ route('demoteUser', ['id' => $user->id]) }}">
                                            {{ method_field('PATCH') }}
                                            {{ csrf_field() }}
                                            <button type="submit" class="btn btn-outline-primary" {{ $user->role->id === 20 ? 'disabled' : '' }}><i class="fad fa-fw fa-arrow-down"></i> Demote</button>
                                        </form>
