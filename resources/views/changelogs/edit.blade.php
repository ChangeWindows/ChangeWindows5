...
<form method="POST" action="{{ route('updateChangelogs', $changelog->id) }}" class="row">
    ...
    <div class="col-2">
        ...
        <div class="list-group list-group-changelogs mt-3">
            <a href="{{ route('updateChangelog', ['id' => 0]) }}" class="list-group-item {{ $changelog->platform == '0' ? 'active' : ''}}">Generic</a>
            <a href="{{ route('updateChangelog', ['id' => 1]) }}" class="list-group-item {{ $changelog->platform == '1' ? 'active' : ''}}">PC</a>
            <a href="{{ route('updateChangelog', ['id' => 2]) }}" class="list-group-item {{ $changelog->platform == '2' ? 'active' : ''}}">Mobile</a>
            <a href="{{ route('updateChangelog', ['id' => 3]) }}" class="list-group-item {{ $changelog->platform == '3' ? 'active' : ''}}">Xbox</a>
            <a href="{{ route('updateChangelog', ['id' => 4]) }}" class="list-group-item {{ $changelog->platform == '4' ? 'active' : ''}}">Server</a>
            <a href="{{ route('updateChangelog', ['id' => 5]) }}" class="list-group-item {{ $changelog->platform == '5' ? 'active' : ''}}">Holographic</a>
            <a href="{{ route('updateChangelog', ['id' => 6]) }}" class="list-group-item {{ $changelog->platform == '6' ? 'active' : ''}}">IoT</a>
            <a href="{{ route('updateChangelog', ['id' => 7]) }}" class="list-group-item {{ $changelog->platform == '7' ? 'active' : ''}}">Team</a>
            <a href="{{ route('updateChangelog', ['id' => 8]) }}" class="list-group-item {{ $changelog->platform == '8' ? 'active' : ''}}">ISO</a>
            <a href="{{ route('updateChangelog', ['id' => 9]) }}" class="list-group-item {{ $changelog->platform == '9' ? 'active' : ''}}">SDK</a>
        </div>
    </div>
    <div class="col-10">
        {{ csrf_field() }}
